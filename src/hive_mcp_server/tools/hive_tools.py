"""Hive tool business logic: config, connection, safety, and tool functions.

Imported by ``server.py``, which registers thin ``@mcp.tool()`` wrappers that
delegate here. Each public function returns a JSON-encoded string so the MCP
layer can hand it directly to the model without additional serialization.
"""
...
def _cfg() -> dict:
    ...
    Read-per-call (rather than at import) lets ``uvx hive-mcp-server`` fail
    with a clear error at run time when creds are missing, instead of
    exploding at import — important because MCP clients import this module
    *before* the subprocess env is fully wired in some setups.
    """
...
def _get_hive_connection():
    ...
    Cloudera Virtual Warehouses expect a Basic-auth ``Authorization`` header
    on a THttpClient transport. Connections are per-call: pyhive connections
    are not thread-safe and MCP tool invocations are short-lived, so pooling
    would add complexity for no measurable benefit at this scale.
    """
    cfg = _cfg()
    _require_creds(cfg)
...
# ---------------------------------------------------------------------------
# Public tool functions  (each returns a JSON string)
# ---------------------------------------------------------------------------


def list_databases() -> str:
    """Return a JSON array of database name strings."""
    rows = _run("SHOW DATABASES")
    return json.dumps([next(iter(row.values())) for row in rows])


def list_tables(database: str) -> str:
    """Return a JSON array of table name strings for the given database."""
...
def describe_table(database: str, table: str) -> str:
    """Return a JSON array of column-descriptor dicts for database.table."""
    db = _quote_ident(database)
    tbl = _quote_ident(table)
    rows = _run(f"DESCRIBE {db}.{tbl}")
    return json.dumps(rows, default=str)


def get_table_sample(database: str, table: str, limit: int = 10) -> str:
    """Return a JSON array of row dicts (up to ``limit`` rows, 1..100)."""
    db = _quote_ident(database)
    tbl = _quote_ident(table)
    if not isinstance(limit, int) or limit < 1 or limit > 100:
        raise ValueError("limit must be an integer between 1 and 100")
    rows = _run(f"SELECT * FROM {db}.{tbl} LIMIT {limit}")
    return json.dumps(rows, default=str)


def execute_query(query: str) -> str:
    """Execute a HiveQL query and return the result as a JSON array of row dicts.

    Rejects write SQL when ``HIVE_READ_ONLY=true`` (default). Caps result rows
    at ``HIVE_QUERY_ROW_LIMIT``.
    """
    cfg = _cfg()
    ...
    rows = _run(query, row_limit=cfg["row_limit"])
    return json.dumps(rows, default=str)
